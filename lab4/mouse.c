...
int start_mouse(){
	if(sys_outb(STAT_REG, 0xA8) == OK) //Enable Mouse
		if(sys_outb(STAT_REG, 0xD4) == OK) //Write Byte to Mouse
			if(sys_outb(OUT_BUF, 0xF4) == OK) //Enable (Data Reporting)
				return 0;
	return -1;
}
...
int end_mouse(){
	if(sys_outb(STAT_REG, 0xD4) == OK) //Write Byte to Mouse
		if(sys_outb(OUT_BUF,0xF5) == OK) //Disable (Data Reporting)
			if(sys_outb(STAT_REG, 0xA7) == OK) //Disable Mouse
				return 0;
	return -1;
}
